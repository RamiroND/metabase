...
(defn should-display-mirror-databases?
  ([] (should-display-mirror-databases? false))
  ([should-include-mirrors?]
   (and (premium-features/enable-database-routing?)
        api/*is-superuser?*
        should-include-mirrors?)))
  ...
  (let [dbs (t2/select :model/Database {:order-by [:%lower.name :%lower.engine]
                                        :where [:and
                                                (when-not include-analytics?
                                                  [:= :is_audit false])
                                                (when-not (should-display-mirror-databases? include-mirror-databases?)
                                                  [:= :primary_database_id nil])]})
        filter-by-data-access? (not (or include-editable-data-model? exclude-uneditable-details?))]
    (cond-> (add-native-perms-info dbs)
      include-tables?              add-tables
      true                         add-can-upload-to-dbs
      true                         (maybe-remove-mirror-databases include-mirror-databases?)
...
(api.macros/defendpoint :get "/"
  ...
  * `include_only_uploadable` will only include DBs into which Metabase can insert new data.
  ...
  database details."
  [_route-params
   {:keys [include saved include_editable_data_model exclude_uneditable_details include_only_uploadable include_analytics include_mirror_databases]}
   :- [:map
...
(defn get-database
  ...
  [id {:keys [include include-editable-data-model? exclude-uneditable-details? include-mirror-databases?]}]
  (let [filter-by-data-access? (not (or include-editable-data-model? exclude-uneditable-details?))
        database               (api/check-404 (if (should-display-mirror-databases? include-mirror-databases?)
                                                (t2/select-one :model/Database :id id)
                                                (t2/select-one :model/Database :id id :primary_database_id nil)))]
    (cond-> database
...
(api.macros/defendpoint :get "/:id"
  ...
  [{:keys [id]} :- [:map
   ...
   :- [:map
       ...
       [:include_mirror_databases {:optional true} ms/BooleanValue]]]
...
(api.macros/defendpoint :get "/:id/usage_info"
  ...
  [{:keys [id]} :- [:map
                    [:id ms/PositiveInt]]]
  (api/check-superuser)
  (api/check-404 (t2/exists? :model/Database {:where
                                              [:and
                                               [:= :id id]
                                               (when-not (should-display-mirror-databases?)
                                                 [:= :primary_database_id nil])]}))
...
(defn- db-metadata [id include-hidden? include-editable-data-model? remove_inactive? skip-fields?]
  (let [db (-> (if include-editable-data-model?
                 (api/check-404 (t2/select-one :model/Database :id id :primary_database_id nil))
                 (api/read-check :model/Database :id id :primary_database_id nil))
               (t2/hydrate
                (if skip-fields?
...
(api.macros/defendpoint :get "/:id/autocomplete_suggestions"
  ...
  (api/read-check :model/Database id :primary_database_id nil)
...
(api.macros/defendpoint :get "/:id/idfields"
  ...
  (let [[db-perm-check field-perm-check] (if (Boolean/parseBoolean include_editable_data_model)
    ...
    (db-perm-check (t2/select-one :model/Database :id id :primary_database_id nil))
...
(api.macros/defendpoint :post "/:id/persist"
  ...
  (api/let-404 [database (t2/select-one :model/Database :id id :primary_database_id nil)]
...
(api.macros/defendpoint :post "/:id/sync_schema"
  ...
  (let [db (api/write-check (api/check-404 (t2/select-one :model/Database :id id :primary_database_id nil)))]
...
(api.macros/defendpoint :post "/:id/rescan_values"
  ...
  (let [db (api/write-check (t2/select-one :model/Database :id id :primary_database_id nil))]
...
(api.macros/defendpoint :get "/:id/syncable_schemas"
  ...
  [{:keys [id]} :- [:map
                    [:id ms/PositiveInt]]]
  (let [db (api/check-404 (t2/select-one :model/Database id :primary_database_id nil))]
    (api/check-403 (mi/can-write? db))
    (->> db
...
(defn database-schemas
  ...
  (let [filter-schemas (fn [schemas]
    ...
    (if include-editable-data-model?
      (api/check-404 (t2/select-one :model/Database id :primary_database_id nil))
      (api/read-check :model/Database id :primary_database_id nil))
